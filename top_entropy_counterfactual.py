...
def generate_counterfactual(z_n,
                            desired_action,
                            agent,
                            P,
                            speed=.01,
    ...
    while True:
        ...
        if (action[0] == desired_action) or i > MAX_ITERS:
            ...
            if diff >= epsilon or i > MAX_ITERS:
                print("selected a {} from pi of {}".format(
                    action[0],
                    logp_cf.exp().cpu().data[0].numpy()))
...
def saliency_on_atari_frame(saliency,
                            atari,
                            fudge_factor=330,
                            channel=0,
                            sigma=.75):
...
def get_low_entropy_states(agent, frames_to_cf, cur_envs, new_frame_bw,
    ...
    state_history = deque(
        [state, state.clone(),
         state.clone(), state.clone()], maxlen=4)
    ...
    while done == False:
        ...
        actions_e = p.max(1)[1].item()
        new_frame, reward, done, _ = env.step(actions_e)
        ...
        state_history.append(
            Variable(torch.Tensor(img).view(1, 1, 80, 80)).cuda())
    ...
    return sorted_entropies[min(frames_to_cf, len(entropies)) -
                            1], all_game_actions
...
def run_game(encoder,
    ...
    _, ddict_ranks = get_low_entropy_states(agent, frames_to_cf, envs,
                                            new_frame_bw, missing, end_frame)
    ...
    agent_state = Variable(
        torch.Tensor(ablate_screen(new_frame_bw, missing)).cuda())
    agent_state_history = deque([
        agent_state,
        agent_state.clone(),
        agent_state.clone(),
        agent_state.clone()
    ...
    while done[0] == False and cf_count < frames_to_cf:
        ...
        agent_state = torch.cat(
            list(agent_state_history),
            dim=1)  #torch.cat(list(agent_state_history), dim=1)
        ...
        agent_state_history.append(
            Variable(
                torch.Tensor(ablate_screen(new_frame_bw, missing)).cuda()))
        state = Variable(torch.Tensor(new_frame_rgb).permute(0, 3, 1,
                                                             2)).cuda()
        ...
        for a in range(envs.get_action_size()):
            ...
            saliency_img = generate_saliency(atari_frame, out_recon, out_cf,
                                             salient_intensity) / 255
            ...
            demo_img = np.hstack([out_state, saliency_img[35:195, :], out_cf
                                  ]) * 255
            text_to_add = "Original action a:                                                                 Saliency, Time Step:                                                      Counterfactual action a': "
            text_to_add2 = "\n{} {: <9}                                                                                 {:04d}                                                                             {} {}".format(
                actions[0], action_description[actions[0]], i, a,
                action_description[a])

            cur_distance = np.linalg.norm(z_n[0].cpu().data.numpy() -
                                          cf_zn[0].cpu().data.numpy())
            file_details = '{:04d}_action{}r{}_cf{}r{}{}.png'.format(
                i, action_description[actions[0]],
                calculate_rank(ddict_ranks, actions[0]), a,
                calculate_rank(ddict_ranks, a), action_description[a])
            ...
            files_to_save1.append(
                (img_dir + '/output1_' + file_details, out_state * 255))
            files_to_save2.append((img_dir + '/output2_' + file_details,
                                   saliency_img[35:195, :] * 255))
            files_to_save3.append(
                (img_dir + '/output3_' + file_details, out_cf * 255))
            files_to_save4.append(
                (img_dir + '/bw_' + file_details, out_cf.mean(2) * 255))
            #save just salient
        ...
        #import pdb; pdb.set_trace()
        immsave(
            *files_to_save[np.argmax(distances)])  #save only the best action
...
def main():
    ...
    img_dir = os.path.join(
        img_dir, "imgs_{}_{}".format(args.enc_file[:-7] + args.enc_file[-2:],
                                     args.agent_file[-7:]))
    ...
    agent = model.Agent(envs.get_action_size(), args.agent_latent).cuda(
    )  #cuda is fine here cause we are just using it for perceptual loss and copying to discrim
    ...
    run_game(encoder, generator, agent, Q, P, envs, seed, img_dir,
             args.missing, frames_to_cf, speed, MAX_ITERS, args.cf_all_actions,
             args.salient_intensity, args.last_frame_diff)
